...
import com.easybus.gui.Object.GoThrough;
...
public class BusHelper extends SQLiteOpenHelper {
	...
	private static String DB_PATH = "/data/data/com.easybus/databases/";
	...
	public void insertToFavorite(String code) {
		String sql = "UPDATE busInfo Set isFavorite ='yes' WHERE code=?";
	...
	public void removeFromFavorite(String code) {
		String sql = "UPDATE busInfo Set isFavorite ='' WHERE code=?";
		SQLiteStatement statement = getReadableDatabase().compileStatement(sql);
		statement.bindString(1, code);
		statement.executeUpdateDelete();
	}
	...
	public Cursor getAll() {
		String sql = "select distinct busLineCode from GoThrough gt,BusStop bs where gt.busStopCode = bs.code and busLineCode in(select busLineCode from GoThrough gt,BusStop bs where gt.busStopCode = bs.code and address_name = '"
				+ "(A) BX Gia Lâm 01"
				+ "') and busLineCode in(select busLineCode from GoThrough gt,BusStop bs where gt.busStopCode = bs.code and address_name = '"
				+ "78-80A Khâm Thiên (Đối diện 71)" + "')";
		Cursor s = getReadableDatabase().rawQuery(sql, null);
		s.moveToNext();
		Log.d("test", s.getString(0));
	...
	public Cursor getAllLines() {
		return getReadableDatabase()
				.rawQuery(
						"SELECT _id,code,name,operationsTime,spacingTime,cost,goRoutePath,goRoutePathGeo,goRouteThroughStops,returnRoutePath,returnRoutePathGeo, returnRouteThroughStops FROM BusLine ORDER BY _id",
						null);
	...
	public Cursor getBusSearch(String txt) {
		String s = "0";
		Cursor c = getReadableDatabase().rawQuery("SELECT * FROM BusLine ",
				null);
		List<String> value = new ArrayList<String>();
		if (c.getCount() > 0) {
			for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
				try {
					//Check if input string is a number? if yes, find bus code
					Integer.parseInt(txt);
					if (convert.ConvertString(this.getCode(c)).contains(
							convert.ConvertString(txt))) {
						s += " OR _id = ?";
						value.add(c.getString(0));
					}
				} catch (NumberFormatException e) {
					//input string is not  a number, find bus name
					if (convert.ConvertString(this.getCode(c)).contains(
							convert.ConvertString(txt))) {
						s += " OR _id = ?";
						value.add(c.getString(0));
					} else if (convert.ConvertString(this.getName(c)).contains(
							convert.ConvertString(txt))) {
						s += " OR _id = ?";
						value.add(c.getString(0));
					}
					// else if
					// (convert.ConvertString(this.getGo(c)).contains(convert.ConvertString(txt)))
					// s += " OR _id = " + c.getString(0);
					// else if
					// (convert.ConvertString(this.getRe(c)).contains(convert.ConvertString(txt)))
					// s += " OR _id = " + c.getString(0);
				}
			}
			
			//if could not find any thing in bus name and bus code, find in go and re route
			if (s.equals("0")) {
				for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
					if (convert.ConvertString(this.getGo(c)).contains(
							convert.ConvertString(txt))) {
						s += " OR _id = ?";
						value.add(c.getString(0));
					} else if (convert.ConvertString(this.getRe(c)).contains(
							convert.ConvertString(txt))) {
						s += " OR _id = ?";
						value.add(c.getString(0));
					}
		...
		return getReadableDatabase().query("BusLine", null, s,
				(String[]) value.toArray(new String[value.size()]), null, null,
				null);
	...
	public int getBusCost(String code) {
		...
		Cursor c = getReadableDatabase().query("BusLine",
				new String[] { "cost" }, "code = ?", new String[] { code },
				null, null, null);
	...
	public Nodes findBusStop(String string) {
		Cursor busStop = getReadableDatabase().query("busStop", null,
				"address_name = ?", new String[] { string }, null, null, null);
		if (busStop.getCount() > 0) {
			...
			Double latitude = Double.parseDouble(busStop.getString(4).split(
					"\\|")[1]);
			Double longitude = Double.parseDouble(busStop.getString(4).split(
					"\\|")[0]);
	...
	public Cursor getItemCondition(String code) {
		return getReadableDatabase().query("busInfo", null,
				"code = ? AND isFavorite = 'yes'", new String[] { code }, null,
				null, null);
	}
	...
	public ArrayList<Nodes> getNodeInfo(String idBus, String direction) {
		...
		// Log.d("ID",idBus);
		if (direction == "go")
			c = getReadableDatabase().query("BusLine",
					new String[] { "goRouteThroughStops" }, "code = ?",
					new String[] { idBus }, null, null, null);
		else
			c = getReadableDatabase().query("BusLine",
					new String[] { "returnRouteThroughStops" }, "code = ?",
					new String[] { idBus }, null, null, null);
		...
		if (c.getCount() > 0) {
			...
			query += "0";
			Cursor busStop = getReadableDatabase().query("BusStop", null,
					query, value.toArray(new String[value.size()]), null, null,
					null);
			if (busStop.getCount() > 0) {
				for (busStop.moveToFirst(); !busStop.isAfterLast(); busStop
						.moveToNext()) {
					String stopName = busStop.getString(2);
					String fleetOver = busStop.getString(1);
					Double latitude = Double.parseDouble(busStop.getString(4)
							.split("\\|")[1]);
					Double longitude = Double.parseDouble(busStop.getString(4)
							.split("\\|")[0]);
					Nodes node = new Nodes(null, stopName);
					node.setFleetOver(fleetOver);
					node.setGeo(new LatLng(latitude, longitude));
					...
					// }
					nodeList.get(busStopIDs.indexOf(busStop.getString(3)))
							.setStopName(stopName);
					nodeList.get(busStopIDs.indexOf(busStop.getString(3)))
							.setFleetOver(fleetOver);
					nodeList.get(busStopIDs.indexOf(busStop.getString(3)))
							.setGeo(new LatLng(latitude, longitude));
	...
	/*
	 * public ArrayList<Nodes> getNodeInfo(String idBus, String direction) {
	 * Cursor c = null; //Log.d("ID",idBus); if (direction=="go") c =
	 * getReadableDatabase().rawQuery(
	 * "SELECT goRouteThroughStops FROM BusLine WHERE _id = "+idBus, null); else
	 * c = getReadableDatabase().rawQuery(
	 * "SELECT returnRouteThroughStops FROM BusLine WHERE _id = "+idBus, null);
	 * ArrayList<Nodes> nodeList = new ArrayList<Nodes>(); if(c.getCount()>0) {
	 * // for(c.moveToFirst();!c.isAfterLast();c.moveToNext()) // {
	 * c.moveToFirst(); String str = c.getString(0); String[] busStopID =
	 * str.split(" "); for(int i=0; i< busStopID.length;i++){ Cursor busStop =
	 * getReadableDatabase().rawQuery( "SELECT * FROM BusStop WHERE code = '"+
	 * busStopID[i]+"'", null); //Log.d("busStop",busStopID[i]);
	 * if(busStop.getCount()>0) { busStop.moveToNext(); String stopName =
	 * busStop.getString(2); String fleetOver = busStop.getString(1);
	 * //Log.d("latitude",busStop.getString(4).split("\\|")[1]);
	 * 
	 * Double latitude =
	 * Double.parseDouble(busStop.getString(4).split("\\|")[1]); Double
	 * longitude = Double.parseDouble(busStop.getString(4).split("\\|")[0]);
	 * Nodes node = new Nodes(null, stopName); node.setFleetOver(fleetOver);
	 * node.setGeo(new LatLng(latitude, longitude)); if (i > 0) {
	 * nodeList.get(nodeList.size() - 1).setNextNode(node); } if (i ==
	 * busStopID.length - 1) { node.setNextNode(null); } nodeList.add(node);
	 * busStop.close(); } } if (nodeList.size() > 0)
	 * nodeList.get(0).setNextNode(nodeList.get(1));
	 * 
	 * 
	 * // } } c.close(); return nodeList; }
	 */
	public Cursor getGeo(String code) {
		return getReadableDatabase()
				.query("busLine",
						new String[] { "goRoutePathGeo", "returnRoutePathGeo",
								"code" }, "code = ?", new String[] { code },
						null, null, null);
	}
	...
	public Cursor getNearBus(List<String> nearBusList) {
		...
		return getReadableDatabase().query("BusLine", null, query,
				value.toArray(new String[value.size()]), null, null, null);
	}
	...
	public Cursor getNearStop(List<String> nearBusList) {
		...
		return getReadableDatabase().query("BusStop", null, query,
				value.toArray(new String[value.size()]), null, null, null);
	}

	public List<GoThrough> findAllGoThrough() {
		List<GoThrough> ls = new ArrayList<GoThrough>();
		Cursor c = getReadableDatabase().rawQuery("SELECT * FROM GoThrough",
				null);
		if (c.getCount() > 0) {
			for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
				ls.add(new GoThrough(c.getString(1), c.getString(2), c
						.getInt(3), c.getInt(4), c.getInt(5)));
			}
		}
		return null;
	}
